...
def get_statistik_keuangan(tahun=None):
    ...
    - Tren bulanan pemasukan dan pengeluaran
    - Distribusi kategori untuk pemasukan dan pengeluaran
    - Total tahunan
    """
    if not tahun:
    ...
    try:
        ...
        for bulan in range(1, 13):
            ...
            tren_bulanan.append({
                'bulan': bulan,
                'pemasukan': str(total_pemasukan),
                'pengeluaran': str(total_pengeluaran)
            })
        
        # Distribusi kategori pemasukan
        distribusi_pemasukan = db.session.query(
        ...
        distribusi_pengeluaran = db.session.query(
            Kategori.nama.label('kategori'),
        ...
        # Total tahunan
        total_tahun_pemasukan = db.session.query(db.func.sum(Pemasukan.jumlah)).filter(
            db.func.year(Pemasukan.tanggal) == tahun
        ).scalar() or 0
        
        total_tahun_pengeluaran = db.session.query(db.func.sum(Pengeluaran.jumlah)).filter(
            db.func.year(Pengeluaran.tanggal) == tahun
        ).scalar() or 0
